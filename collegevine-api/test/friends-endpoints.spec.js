...
describe('Friends Endpoints', function () {
  ...
  describe(`Unauthorized requests`, () => {
    ...
    it('responds with 401 Unauthorized for GET /api/colleges/:user_email', () => {
      return supertest(app)
        .get('/api/colleges/:user_email')
        .expect(401, { error: 'Missing bearer token' });
    });
  });

  describe(`GET /api/colleges/:user_email`, () => {
    const testUsers = helpers.makeUsersArray();
    ...
    context(`Given no users in database with matching email`, () => {
      it(`responds with 404`, () => {
        ...
        return supertest(app)
          .get(`/api/colleges/${nonExistentEmail}`)
          .set('Authorization', helpers.makeAuthHeader(testUser))
          .expect(404, { error: `Can't find user with email` });
      });
    });
    ...
    context('Given there is a matching email in the database', () => {
      ...
      it('responds with 200 and the colleges from user email id', () => {
        const existingUser = testUsers[1];
        const expectedProperties = helpers.makeExpectedPropertiesForFriend(
          existingUser.id,
          testProperties,
        );

        return supertest(app)
          .get(`/api/colleges/${existingUser.email}`)
          .set('Authorization', helpers.makeAuthHeader(testUser))
