...
describe('Reservations Endpoints', function () {
  ...
  describe(`Unauthorized requests`, () => {
    ...
    it(`responds with 401 Unauthorized for DELETE /api/reservations/:reservation_id`, () => {
      ...
      return supertest(app)
        .delete(`/api/colleges/:reservation_id`)
        .expect(401, { error: 'Missing bearer token' });
    });
  });
  ...
  describe(`GET /api/reservations/user`, () => {
    ...
    const testProperties = helpers.makePropertiesArray(testUsers);
    const testReservations = helpers.makeReservationsArray(
      testUsers,
      testProperties,
    );
    ...
    context('Given there are reservations in the database', () => {
      beforeEach('insert reservations', () =>
        helpers.seedReservations(db, testProperties, testReservations),
      );

      it('responds with 200 and a count of reservations', () => {
        const expectedReservations = helpers.makeExpectedReservationCount(
          testUser,
          testProperties,
          testReservations,
        );
  ...
  describe(`GET /api/reservations/property/:property_id`, () => {
    ...
    context('Given there are reservations in the database', () => {
      ...
      it('responds with 200 and reservations for the specified property', () => {
        const testCollegeId = testProperties[1].id;
        const expectedReservations = helpers.makeExpectedReservationsForCollege(
          testCollegeId,
          testReservations,
        );

        return supertest(app)
          .get(`/api/reservations/property/${testCollegeId}`)
          .set('Authorization', helpers.makeAuthHeader(testUser))
          .expect(200, expectedReservations);
      });
    });
    ...
    context(`Given an XSS attack reservation`, () => {
      const testCollege = testProperties.find((p) => p.user_id === testUser.id);

      beforeEach('insert colleges', () =>
        helpers.seedProperties(db, testProperties),
      );

      const { maliciousReservation, expectedReservation } =
        helpers.makeMaliciousReservation(testUser, testCollege);

      beforeEach('insert malicious reservation', () => {
        return helpers.seedMaliciousReservation(db, maliciousReservation);
      });

      it('removes XSS attack content', () => {
        return supertest(app)
          .get(`/api/reservations/property/${testCollege.id}`)
          .set('Authorization', helpers.makeAuthHeader(testUser))
          .expect(200)
          .expect((res) => {
  ...
  describe(`GET /api/reservations/reservation/:reservation_id`, () => {
    ...
    context('Given there are reservations in the database', () => {
      ...
      it('responds with 200 and details about reservation', () => {
        ...
        const expectedReservationDetails =
          helpers.makeExpectedReservationDetails(testUsers, testReservation);
  ...
  describe(`POST /api/reservations`, () => {
    ...
    beforeEach(() => helpers.seedProperties(db, testProperties));
  ...
  describe(`GET /api/reservations`, () => {
    ...
    context('Given there are reservations in the database', () => {
      ...
      it('responds with 200 and all of the reservations', () => {
        const expectedReservations = testReservations.map((reservation) => {
          const { id, property_id, user_id, title, all_day, date_created } =
            reservation;
          return {
            id,
            property_id,
    ...
    //     return helpers.seedMaliciousCollege(
    //       db,
    //       maliciousCollege,
    //     )
    //   })
