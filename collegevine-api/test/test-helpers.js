...
function makeReservationsArray(users, colleges) {
  return [
    {
      id: 1,
      property_id: colleges[0].id,
      user_id: users[0].id,
...
function makeExpectedProperties(user, colleges) {
  const userProperties = colleges.filter(
...
function makeMaliciousCollege(user) {
  const maliciousCollege = {
    id: 911,
  ...
  const expectedCollege = {
    ...makeExpectedProperties(user, [maliciousCollege])[0],
    address:
...
function cleanTables(db) {
  return db.raw(
    `TRUNCATE
      collegevine_users,
      collegevine_colleges,
      collegevine_reservations
...
function seedUsers(db, users) {
  ...
  return db
    .into('collegevine_users')
    .insert(preppedUsers)
    .then(() =>
      // update the auto sequence to stay in sync
      db.raw(`SELECT setval('collegevine_users_id_seq', ?)`, [
        users[users.length - 1].id,
      ]),
    );
}

function seedMaliciousCollege(db, property) {
  return db.into('collegevine_colleges').insert(property);
}
...
function seedProperties(db, colleges) {
  return db.into('collegevine_colleges').insert(colleges);
}

function seedReservations(db, colleges, reservations = []) {
  // use a transaction to group the queries and auto rollback on any failure
  return db.transaction(async (trx) => {
    // await trx.into('collegevine_users').insert(users)
    await trx.into('collegevine_colleges').insert(colleges);
    // update the auto sequence to match the forced id values
    await Promise.all([
      // trx.raw(
      //   `SELECT setval('collegevine_users_id_seq', ?)`,
      //   [users[users.length - 1].id],
      // ),
      trx.raw(`SELECT setval('collegevine_colleges_id_seq', ?)`, [
        colleges[colleges.length - 1].id,
      ]),
    ]);
    // only insert reservations if there are some, also update the sequence counter
    if (reservations.length) {
      await trx.into('collegevine_reservations').insert(reservations);
      await trx.raw(`SELECT setval('collegevine_reservations_id_seq', ?)`, [
...
function makeExpectedReservationCount(user, colleges, reservations) {
  const count = {
    owned: null,
    made: null,
  };

  count.made = reservations.filter(
    (reservation) => reservation.user_id === user.id,
  ).length;

  const propertyIds = colleges
    .filter((property) => property.user_id === user.id)
    .map((p) => p.id);

  count.owned = reservations.filter((reservation) =>
    propertyIds.includes(reservation.property_id),
  ).length;
...
function makeMaliciousReservation(user, property) {
  ...
  const expectedReservation = {
    ...makeExpectedReservationsForCollege(user, [maliciousReservation])[0],
    title:
      'Naughty naughty very naughty &lt;script&gt;alert("xss");&lt;/script&gt;',
  };
  return {
...
function makeExpectedReservationsForCollege(propertyId, reservations) {
  return reservations
    .filter((reservation) => reservation.property_id === propertyId)
    .map((filteredRes) => {
      const { all_day, date_created, id, property_id, title, user_id } =
        filteredRes;
      return {
        all_day,
        date_created,
...
function seedMaliciousReservation(db, reservation) {
  return db.into('collegevine_reservations').insert(reservation);
}
...
function makeExpectedPropertiesForFriend(id, colleges) {
  return colleges
...
module.exports = {
  ...
  makeMaliciousCollege,
  seedMaliciousCollege,
  seedReservations,
  makeExpectedReservationCount,
  makeMaliciousReservation,
  makeExpectedReservationsForCollege,
